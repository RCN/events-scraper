...
const extractRacerInfo = ({response, urlWithContext}) => {
  ...
  if (!name && !age && !address && !raceCount) {
    log.doneBut('#' + urlWithContext.context.licenceNo + ' ' + 'All fields are empty, something is wrong.')
    return []
  }
...
const buildScraperForLicenses = ({
  ...
  return buildScraper({
    fileName: `racers_lic-no-npm range_${licenceNoFrom}-${licenceNoTo}`,
...
crawlNStartingFrom({startingLicNo: 0, 
  amountToCrawl: 1000, delay: 1000, concurrency: 3})

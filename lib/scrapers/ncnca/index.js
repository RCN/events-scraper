...
import { scrapeNcncaEventsWiaWebLinksFrom } from './web-events-scraper'
...
async function scrapeNcncaEvents({ year }) {
  ...
  const { data: additionalEventProperties } = await scrapeNcncaEventsWiaWebLinksFrom(
    origEvents
  )
...
async function main() {
  try {
    // const { data: events } = await scrapeNcncaEvents({ year: 2017 })
    const { data: events } = await scrapeNcncaEvents({ year: 2018 })
    console.dir(events.length, { colors: true, depth: 4 })

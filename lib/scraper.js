...
const requestWithRotatingUserAgent = (url, successStatusCodes) => {
  const userAgents = [
  ...
  return request(url, options)
    .then(response => {
      //response is an array and
      if (!successStatusCodes.includes(response.statusCode)) {
        throw new Error(`Request status code was "${response.statusCode}" and is not one of [${successStatusCodes}]`,
          response)
      }
  ...
  return new Bluebird.Promise((resolve, reject) => {
    const attempt = (attemptNo) => {
      ...
      funcToRetry(attemptNo)
        ...
        .catch((err) => {
          if (attemptNo >= max) {
            log.fail(`${operationInfo} – Failed after ${max} attempts :(`)
            return reject(err)
          }
...
const buildScraper = ({
  ...
  return urlsWithContext =>
    Bluebird.map(
      urlsWithContext,
      // async function (urlWithContext) {
      //   const response = await retry(() => requestWithRotatingUserAgent(urlWithContext.url, successStatusCodes), {
      //     max: retryAttempts,
      //     backoff: retryDelay,
      //     operationInfo: urlWithContext.url,
      //   })
      //
      //   await Bluebird.delay(randomizeDelay ? rnd(delay / 2, delay * 2) : delay)
      //
      //   const data = scrapingFunc({
      //     response: response,
      ...
      //
      //     await appendJsonToFile(`data/cache/${cacheFileName}`, data, {spaces: 2})
      //   }
      //
      //   return data
      // },
      urlWithContext =>
        retry(() => requestWithRotatingUserAgent(urlWithContext.url, successStatusCodes), {
          max: retryAttempts,
          backoff: retryDelay,
          operationInfo: urlWithContext.url,
        })

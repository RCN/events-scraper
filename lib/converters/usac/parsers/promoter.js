...
const createPromoterParser = existingPromoters => {
  const promotersByClubId = keyBy('usacClubId')(existingPromoters)
  ...
  return promoterRaw => {
    ...
    if (matchingPromoter) {
      const levenDistance = leven(
        toLower(promoterRaw.club),
        toLower(matchingPromoter.name)
      )

      if (levenDistance >= 3) {
        log.warn(
          `Found matching prmoter for usac clubId: ${clubId}, but promoter names ` +
            `do not match, USAC name: "${promoterRaw.club}", RCN name: "${
              matchingPromoter.name
            } \n"`
        )
      }
    } else {
      ...
      log.error(
        `NO MATCH, club: ${promoterRaw.club}, name: ${
          promoterRaw.raceDirector
        }, id: ${clubId}` +
...
module.exports = createPromoterParser

...
class UsaCyclingScraper {
  ...
  toAbsoluteUsacUrl(url) {
    ...
    return url.startsWith('http') ? url : Url.resolve('https://usacycling.org', url)
  }
...
const getEventTypeByImagePath = ({ imagePath }) => {
  if (!imagePath) {
    ...
    throw new Error(
      `Can't get event type by image path, unknown image path: "${imagePath}"`
    )
  }
}
...
export default function extractEvents({ response, urlWithContext }) {
  ...
  $events.each((index, eventElement) => {
    ...
    const $linksInDetailsDivTd0 = $eventDetailsDiv
      .find('table td')
      .eq(0)
      .find('a')
    const $linksInDetailsDivTd1 = $eventDetailsDiv
      .find('table td')
      .eq(1)
      .find('a')
    ...
    const dates =
      scraper.scrapeDate($eventDetailsDiv.find('table td').get(0)) ||
    ...
    let event = {
      ...
      promoter: {
        club: $linksInDetailsDivTd1
          .eq(0)
          .text()
          .trim(),
        raceDirector: $linksInDetailsDivTd1
          .eq(1)
          .text()
          .trim(),
      ...
      usacEventType:
        scraper.getFriendlyUsacTypeNames(context.usacEventType) ||

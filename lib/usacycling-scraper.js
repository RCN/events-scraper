...
//class containing all the scrapers
var Scraper = function(context) {
    this.context = context;
};

Scraper.prototype.scrapeCategory = function(tdContainingCategory) {
    var category = '';
...
Scraper.prototype.scrapeDate = function(tdContainingCategory) {
    for (var i = 0; i < tdContainingCategory.children.length; i++) {
        if (tdContainingCategory.children[i].type == 'text') {
    ...
    log.error('scrapeDate failed for:');
...
var extractEvents = function(params) {
    ...
    var scraper = new Scraper(params.context);

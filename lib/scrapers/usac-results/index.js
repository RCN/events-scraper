...
async function scrapeResults({ year, csvType, from, to }) {
  ...
  const results = await scrape(
    createResultUrls({ year: year, csvType: csvType, from, to })
  )
...
async function main() {
  ...
  // await scrapeResults({ year: 2006, from: 0, to: 2360, csvType: 'plain' })
  // await scrapeResults({ year: 2007, from: 0, to: 2360, csvType: 'plain' })

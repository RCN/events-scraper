...
const buildUrls = ({
  ...
  const eventsSearchUrl = {
    url: 'https://legacy.usacycling.org/events/state_search.php?',
    context: {
      year: null,
      state: null,
  ...
  const lowEventCountStates = []
  // [
  //   'AZ', 'NV', 'OR', 'WA', 'UT', 'ID', 'MT', 'WY', 'NM', 'OK', 'KS', 'NE',
  //   'SD', 'ND', 'MN', 'MO', 'AR', 'LA', 'MS', 'IL', 'WI', 'AL', 'GA', 'SC', 'KY', 'VA', 'WV',
  //   'OH', 'IN', 'MI', 'DC', 'MD', 'DE', 'CT', 'RI', 'MA', 'VT', 'NH', 'ME', 'HI', 'AK'
  // ] // ctn=40
  const mediumEventCountStates = []
    // ['CS', 'NJ', 'FL', 'CO', 'NC', 'IA', 'TX'] // ctn=60
  const highEventCountStates = []
    //['PA'] // ctn=100
  const extraHightEventcountStates = ['CN'] // ctn=120

  // agax=2 is a hardcoded value, which tells to return data as HTML piece and not full HTML page
  let baseUrls = buildUrlsFor([eventsSearchUrl], {urlParam: 'ajax', values: [2]})
  let urlsWithYears = buildUrlsFor(baseUrls, {
    urlParam: 'fyear',
    contextParam: 'year',
  ...
  const buildUrlsForStatesWithCtn = (urls, states, ctn) => {
    const buildUrlsForRankingEvents = urls => {
      ...
      newUrls = buildUrlsFor(newUrls, {urlParam: 'rrfilter', contextParam: 'usacEventType', values: [rrfilter]})
  ...
  const urlsForLowEventCountStates = buildUrlsForStatesWithCtn(urlsWithYears, lowEventCountStates, [0, 20, 40])
  const urlsForMidEventCountStates = buildUrlsForStatesWithCtn(urlsWithYears, mediumEventCountStates, [0, 20, 40, 60])
  const urlsForHighEventCountStates = buildUrlsForStatesWithCtn(urlsWithYears, highEventCountStates, [0, 20, 40, 60, 80, 100])
  const urlsForExtraHighEventCountStates = buildUrlsForStatesWithCtn(urlsWithYears, extraHightEventcountStates, [0, 20, 40, 60, 80, 100, 120])

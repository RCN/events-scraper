...
async function scrapeResults({ year, from, to }) {
  ...
  const results = await scrape(
    createResultUrls({ year: year, csvType: 'plain', from, to })
  )
...
async function main() {
  // await scrapeResults({ year: 2009 })
  const [from, to] = [0, 4300]
  await scrapeResults({ year: 2013, from, to })
  await scrapeResults({ year: 2014, from, to })
  // await scrapeResults({ year: 2012, from, to}) // 4000+
}

...
// TOOD: url for cross-referencing event types: https://www.usacycling.org/events/?advanced=1
...
class UsaCyclingScraper {
  ...
  _parseDates(datesString) {
    let rawDates
    let multiDayEvent = datesString.contains(' - ')
  ...
  toAbsoluteUrl(url) {
    ...
    return (url.startsWith('http')) ? url : Url.resolve('http://usacycling.org', url)
  }
...
export default function extractEvents({response, urlWithContext}) {
  ...
  $events.each((index, eventElement) => {
    ...
    // doable it's not worth the trouble
    // const promoterContactEmail = $eventDetailsDiv.find('table td').eq(1).find('span')
    ...
    let event = {
      ...
      promoter: {
        ...
        url: scraper.toAbsoluteUrl($linksInDetailsDivTd1.eq(0).attr('href')),
      },

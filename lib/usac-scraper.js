...
const fileName = 'non-compeitive_all-years_all-states_all-types_noncomp-1'
...
const buildUrlsForStatesWithCtn = (urls, states, ctn) => {
  const buildUrlsForRankingEvents = (urls) => {
    ...
    newUrls = buildUrlsFor(newUrls, {urlParam: 'noncomp', contextParam: 'noncomp', values: [1]})
    return newUrls
  }
...
let urlsForHighEventCountStates = buildUrlsForStatesWithCtn(urlsWithYears, highEventCountStates, [0, 20, 40, 60])
let urlsForExtraHighEventCountStates = buildUrlsForStatesWithCtn(urlsWithYears, extraHightEventcountStates, [0, 20, 40, 60])

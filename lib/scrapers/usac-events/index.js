...
async function scrapeUsacEvents({ year, state, rrfilter }) {
  ...
  log.task('Scraping...')
  const events = await scrapeEventsFrom(urls)
  return events
}

async function main({ year }) {
  const allEvents = await scrapeUsacEvents({ year: year, state: 'CN', rrfilter: 'all' })
  const funEvents = await scrapeUsacEvents({ year: year, state: 'CN', rrfilter: 'F' })
  ...
  const complementedEvents = RS.replaceBy((a, b) => a.permit === b.permit, funEvents, allEvents)

...
const statesRegex = new RegExp(
  '(AK|AL|AR|AZ|CA|CO|CT|DC|DE|FL|GA|GU|HI|IA|ID|IL|IN|KS|KY|LA|MA|MD|ME|MI|MN|MO|MS|MT|NC|ND|NE|NH|NJ|NM|NV|NY|OH|OK|OR|PA|RI|SC|SD|TN|TX|UT|VA|VI|VT|WA|WI|WV|WY)'
)
...
const parseCityState = cityStateRaw => {
  ...
  if (!city || city.length <= 2) {
    throw new Error(
      `City was not found in the following locaiton.cityStae: ${cityState}`
    )
  }
  if (!state || !state.match(statesRegex)) {
    throw new Error(
      `State was not found in the following locaiton.cityStae:  ${cityState}`
    )
  }
  if (state.length !== 2) {
    throw new Error(
      `State must be of lenth 2, but was ${state.length}, value: "${state}",` +
        `locaiton.cityStae:  ${cityState}`
    )
  }

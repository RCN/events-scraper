...
const createUrlsWithNames = ({ usacEvents, year }) => {
  ...
  const createUrl = name =>
    `https://www.bikereg.com/api/search?name=${encodeURIComponent(
      name
    )}&year=${year}`

  const names = pipe(map(x => x.name), uniq)(usacEvents)
...
async function runScraping() {
  const results = await scrape({
    fromUrls: createUrlsWithPermits({ usacEvents: usac2017Events, year: 2017 })
  })
  ...
  const notFoundUsacEvents = notFoundEventPermits.map(
    permit => usacEventsMap[permit]
  )
  const additionalResults = await scrape({
    fromUrls: createUrlsWithNames({
      usacEvents: notFoundUsacEvents,
      year: 2017
    })
  })

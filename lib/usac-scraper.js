...
// urls = buildUrlsFor(urls, {urlParam: 'rrfilter', contextParam: 'usacEventType', values: ['rr', 'F', '']})
...
req(eventsSearchUrl.url + '?state=CN&race=Road&fyear=&rrfilter=rr')
  ...
  .then(events => {
    ...
    jsonfile.writeFile('data/events.json', events, {spaces: 2}, (err) => {
      if (err) throw err
      log.done('Saved events to "data/events.json"')
    })
  })
  .catch(err => {
    if (err.stack) {

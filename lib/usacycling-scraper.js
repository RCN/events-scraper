...
var parseDates = function(dateText, year) {
    var date = new Date(dateText);

    if (date.toString() == 'Invalid Date') {
        log.dataError(dateText);
        return dateText;
    }
...
var extractEvents = function(data) {
    var $ = cheerio.load(data.response.body);
    ...
    $events.each(function each(index, eventElement) {
        ...
        var event = {
            ...
            date: parseDates($eventDetailsSpan.find('span').text()),
    ...
    var formattedMessage = '[{0} events] '.format(eventsCount) + data.response.request.uri.href;
